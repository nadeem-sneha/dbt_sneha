...
SELECT calendar.month_start_date, c.clustername , count(c.id) AS anc_open_count,
CASE 
...
FROM calendar
CROSS JOIN  
(SELECT  id,clustername, anc_identify_date,anc_close_date,anc_closed,lmpdate FROM {{ref('anc_case')}}) c 
...
GROUP BY calendar.month_start_date,c.clustername,trimester
ORDER BY calendar.month_start_date,c.clustername,trimester