...
FROM {{ref('remove_zzz_case_duplicates')}} AS c
LEFT JOIN
(select caseid,max(visitdate) as lastvisitdate from {{ref('remove_anc_duplicates')}} where visitreason ='ANC' group by caseid ) AS form 
ON form.caseid = c.id
